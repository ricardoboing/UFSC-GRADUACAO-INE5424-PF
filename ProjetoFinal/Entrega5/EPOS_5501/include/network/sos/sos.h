...
protected:
    ...
    struct Packet {
        unsigned int port_destination = 1;
        unsigned int id = 0;
        unsigned int size = 0;
...
public:
    ...
    public:
        SOS_Communicator(unsigned int port);
        ~SOS_Communicator();
    ...
    protected:
        ...
        List<Client>* clients;
        List<Packet>* packet_receive;
        List<Packet>* packet_not_ack;

        Client* client(NIC_Address& address);
    ...
    void send(const NIC_Address& address, Packet* packet);
