...
// Class attributes
i82557a::Device i82557a::_devices[UNITS];

// Class Methods
void i82557a::int_handler(const IC::Interrupt_Id & interrupt)
{
    db<i82557a>(TRC) << "i82557a::int_handler" << endl;

    i82557a * dev = get_by_interrupt(interrupt);

    db<i82557a>(TRC) << "i82557a::int_handler(int=" << interrupt << ",dev=" << dev << ")" << endl;
    ...
    if(!dev)
        db<i82557a>(WRN) << "i82557a::int_handler: handler not found" << endl;
    else
        dev->handle_int();
}
...
i82557a::~i82557a()
{
    db<i82557a>(TRC) << "~i82557a(unit=" << _unit << ")" << endl;
}

i82557a::i82557a(unsigned int unit, const Log_Addr & io_mem, const IO_Irq & irq, DMA_Buffer * dma_buf)
{
    db<i82557a>(TRC) << "i82557a(unit=" << unit << ",io=" << io_mem << ",irq=" << irq << ",dma=" << dma_buf << ")" << endl;
    ...
    db<i82557a> (TRC) << "i82557a(_rx_ring malloc of " << RX_BUFS << " units)" << endl;
    ...
    db<i82557a> (TRC) << "i82557a(_tx_ring malloc of " << TX_BUFS << " units)" << endl;
    // TxCBs
    for(i = 0; i < TX_BUFS; i++) {
        log += align128(sizeof(Tx_Desc));
        phy += align128(sizeof(Tx_Desc));
...
void i82557a::reset()
{
    db<i82557a>(TRC) << "i82557a::reset (software reset and self-test)" << endl;
    ...
    _address[4] = eeprom_mac_address(4);
    _address[5] = eeprom_mac_address(5);
    db<i82557a>(INF) << "i82557a::reset():MAC=" << _address << endl;
...
int i82557a::send(const Address & dst, const Protocol & prot, const void * data, unsigned int size)
{
    // wait for a free TxCB
    while(!(_tx_ring[_tx_cur].status & cb_complete)) {
        if (_tx_cuc_suspended) {
    ...
    // new (_tx_ring[_tx_cur].frame) Frame(_address, dst, htons(prot), data, size); // original
    /* New implementation, double copy: parameters to buffer and buffer to Tx_Desc.frame.
     * Making Tx_Desc.frame a pointer does not work (I believe i82557a demands for
     * a contiguous allocated memory for Tx ring descriptors).
     */
    Buffer * buf = _tx_buffer[_tx_cur];
    Frame * frame = buf->frame();
    ...
    db<i82557a>(TRC) << "i82557a::send(dst=" << dst << ", prot=" << prot << ", data=" << (char *) data << ", size=" << size << ")";
    db<i82557a>(INF) << "i82557a::send:frame={dst=" << frame->dst() << ", prot=" << frame->prot() << ", data=" << (char *) frame->data<void>() << ", size=" << buf->size() << "}" << endl;
...
bool i82557a::verifyPendingInterrupts(void)
{
    if (_rx_ruc_no_more_resources) {
        unsigned short i;
...
int i82557a::receive(Address * src, Protocol * prot, void * data, unsigned int size)
{
    // wait until the RFD is filled up by the device
    while(!(_rx_ring[_rx_cur].status & cb_complete))
        verifyPendingInterrupts();
    ...
    *prot = ntohs(frame->header()->prot());
    ...
    if (! (_rx_ring[_rx_cur].status & RFD_OK_MASK))
        db<i82557a>(WRN) << "Error on frame reception" << endl;
 ...
 * that use this object and the access to _tx_cur is not atomic. */
// TODO: solve this!
i82557a::Buffer * i82557a::alloc(const Address & dst, const Protocol & prot, unsigned int once, unsigned int always, unsigned int payload)
{
    db<i82557a>(TRC) << "i82557a::alloc(s=" << _address << ",d=" << dst << ",p=" << hex << prot << dec << ",on=" << once << ",al=" << always << ",ld=" << payload << ")" << endl;

    int max_data = MTU - always;

    if((payload + once) / max_data > TX_BUFS) {
        db<i82557a>(WRN) << "i82557a::alloc: sizeof(Network::Packet::Data) > sizeof(NIC::Frame::Data) * TX_BUFS!" << endl;
        return 0;
    }
    ...
    for(int size = once + payload; size > 0; size -= max_data) {
        ...
        db<i82557a>(INF) << "i82557a::alloc:desc[" << _tx_cur << "]=" << desc << " => " << *desc << endl;
...
void i82557a::free(Buffer * buf)
{
    db<i82557a>(TRC) << "i82557a::free(buf=" << buf << ")" << endl;
    ...
    for(Buffer::Element * el = buf->link(); el; el = el->next()) {
        ...
        db<i82557a>(INF) << "i82557a::free:desc=" << desc << " => " << *desc << endl;
    }
}

/*! NOTE: this method is not thread-safe because _tx_buffer_prev is shared by
 * all threads that use this object. */
int i82557a::send(Buffer * buf)
{
    db<i82557a>(TRC) << "i82557a::send new api" << endl;
    /// assumes: buf->is_locked();
    ...
    for(Ethernet::Buffer::Element * el = buf->link(); el; el = el->next()) {
        ...
        db<i82557a>(TRC) << "i82557a::send(buf=" << buf << ")" << endl;

        desc->tcb_byte_count = buf->size() + sizeof(Ethernet::Header);

        db<i82557a>(TRC) << "i82557a::send-zc:\n" << "(dst=" << frame->dst() << ", prot=" << frame->prot() << ", data=" << (char *) frame->data<void>() << ", size=" << buf->size() << ")" << endl;
        ...
        db<i82557a>(INF) << "i82557a::send:desc=" << desc << " => " << *desc << endl;
        ...
        db<i82557a>(TRC) << "i82557a::send will unlock" << endl;

        _tx_buffer_prev = buf;
        buf->unlock();

        db<i82557a>(TRC) << "i82557a::send unlocked" << endl;
    }

    db<i82557a>(TRC) << "i82557a::send size=" << size << endl;

    return size;
}

unsigned short i82557a::eeprom_read(unsigned short *addr_len, unsigned short addr) {
...
unsigned char i82557a::eeprom_mac_address(Reg16 addr) {
    Reg16 addr_len = 8;
    Reg16 two_words; // two words of 8 bits (one EEPROM word)
    Reg8 which_word; // first or second word of 8 bits
...
void i82557a::handle_int() {
    CPU::int_disable();
    // IC::disable(IC::irq2int(_irq));

    db<i82557a>(TRC) << "i82557a::handle_int()" << endl;
    db<i82557a>(TRC) << ">" << endl;
    ...
    if ((stat_ack != NOT_OURS) && (stat_ack != NOT_PRESENT)) {
        ...
        for(int count = RX_BUFS; count && (_rx_ring[_rx_cur].status & cb_complete); count--, ++_rx_cur %= RX_BUFS) {
            db<i82557a>(TRC) << "@ count = " << count << ", _rx_cur = " << _rx_cur << endl;
            ...
            if(_rx_buffer[_rx_cur]->lock()) { // if it wasn't, let's handle it
                ...
                if (_rx_ring[_rx_cur].actual_count & (RFD_EOF_MASK | RFD_F_MASK)) {
                    ...
                    db<i82557a>(WRN) << "HDS size" << endl;
                }
                else if (! (_rx_ring[_rx_cur].actual_count & RFD_F_MASK)) {
                    db<i82557a>(WRN) << "Invalid RFD" << endl;
                    // Workaround if QEMU patch not applied
                    // http://patchwork.ozlabs.org/patch/662355/
                    db<i82557a>(WRN) << "Assuming size to be 1500" << endl;
                    size = 1500;
                    // ----
                }
                buf->size(size);
                ...
                db<i82557a>(INF) << "i82557a::int:receive desc_frame(s=" << desc_frame->src() << ",d=" << desc_frame->dst() << ",p=" << hex << desc_frame->prot() << dec << ",t=" << (char *) desc_frame->data<void>() << ",s=" << buf->size() << ")" << endl;

                new (frame) Frame(desc_frame->src(), desc_frame->dst(), desc_frame->prot(), desc_frame->data<void>(), buf->size()); // TODO: FIXME. That is creating a copy on a Zero-copy implementation. :P

                db<i82557a>(INF) << "i82557a::int:receive(s=" << frame->src() << ",d=" << frame->dst() << ",p=" << hex << frame->header()->prot() << dec << ",t=" << (char *) frame->data<void>() << ",s=" << buf->size() << ")" << endl;

                db<i82557a>(INF) << "i82557a::handle_int:desc[" << _rx_cur << "]=" << desc << " => " << *desc << endl;
                ...
                db<i82557a>(TRC) << "Will notify!" << endl;
                if(!notify(frame->header()->prot(), buf)) { // No one was waiting for this frame, so let it free for receive()
                    free(buf);
                    db<i82557a>(TRC) << "Not notified!" << endl;
                }
                else {
                    db<i82557a>(TRC) << "Notified!" << endl;
                }
            }
        }
    }
...
void i82557a::i82557a_configure(void)
{
    configCB->command = cb_config;
    memset(&(configCB->config), 0, sizeof(struct config));
    configCB->config.byte_count = 0x16;              /* bytes in this struct */
...
int i82557a::self_test()
{
    ...
    // Check results of self-test
    if(dmadump->selftest.result != 0) {
        db<i82557a>(WRN) << "i82557a:self_test() => failed with code " << dmadump->selftest.result << "!" << endl;
        return -1;
    } else {
        db<i82557a>(TRC) << "i82557a:self_test() => PASSED with code " << dmadump->selftest.result << "!" << endl;
    }

    if(dmadump->selftest.signature == 0) {
        db<i82557a>(WRN) << "i82557a:self_test() => timed out!" << endl;
        return -1;
    }
