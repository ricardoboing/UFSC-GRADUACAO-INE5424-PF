...
void i82557a::init(unsigned int unit)
{
    db<Init, i82557a>(TRC) << "i82557a::init(unit=" << unit << ")" << endl;
    ...
    PCI::Locator loc = PCI::scan(PCI_VENDOR_ID, PCI_DEVICE_ID, unit);
    if(!loc) {
        db<Init, i82557a>(WRN) << "i82557a::init: PCI scan failed!" << endl;
        return;
    }
    ...
    PCI::header(loc, &hdr);
    if(!hdr) {
        db<Init, i82557a>(WRN) << "i82557a::init: PCI header failed!" << endl;
        return;
    }
    db<Init, i82557a>(INF) << "i82557a::init: PCI header=" << hdr << endl;
    if(!(hdr.command & PCI::COMMAND_MEMORY))
        db<Init, i82557a>(WRN) << "i82557a::init: I/O memory unaccessible!" << endl;
    if(!(hdr.command & PCI::COMMAND_MASTER))
        db<Init, i82557a>(WRN) << "i82557a::init: not master capable!" << endl;

    // Get I/O base port
    Log_Addr io_mem = hdr.region[PCI_REG_MEM].log_addr;

    db<Init, i82557a>(INF) << "i82557a::init: I/O memory at "
                        << hdr.region[PCI_REG_MEM].phy_addr
                        << " mapped to "
                        << hdr.region[PCI_REG_MEM].log_addr
                        << " io_mem=" << io_mem << endl;
    ...
    // Get I/O irq
    IO_Irq irq = hdr.interrupt_line;
    db<Init, i82557a>(INF) << "i82557a::init: PCI interrut pin "
                        << hdr.interrupt_pin << " routed to IRQ "
                        << hdr.interrupt_line << endl;
